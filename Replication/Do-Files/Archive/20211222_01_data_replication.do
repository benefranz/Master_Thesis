...
/*
Changes:
	- generate fueling_behaviour as weights using fueling behaviour
	- generate weighted means of fuel prices
*/
...
forvalues ii=15/30{
	...
	save "$data_out/Prices/2020-06-`i'-prices.dta", replace
}
...
forvalues ii=01/31{
	...
	save "$data_out/Prices/2020-07-`i'-prices.dta", replace
}

* Apend Data
use "$data_out/Prices/2020-06-15-prices.dta", clear
forvalues mm = 16/30 {
	local m : di %02.0f `mm'	
	append using "$data_out/Prices/2020-06-`m'-prices.dta"
}
forvalues oo = 01/31{ 
	local o : di %02.0f `oo'	
	cap append using "$data_out/Prices/2020-07-`o'-prices.dta"
}
...
* Save
save "$data_out/01_prices_germany_hourly.dta", replace
...
* Save
save "$data_out/01_prices_germany_weighted.dta", replace
...
* Save
save "$data_out/01_prices_germany_daily.dta", replace



*-----		 	  1.1.2 Le Prix Des Carburants (French Data)	   		  -----*
...
* Save
save "$data_out/02_frenchdata_2020_hourly.dta", replace
...
* Save
save "$data_out/02_frenchdata_2020_daily.dta", replace
...
foreach c in "DE" "FR"{
		...
		save "$data_out/Mobility/2020_`c'_Region_Mobility_Report.dta", replace
}
...
* Save
save "$data_out/03_communes_dep.dta", replace
...
* Save
save "$data_out/04_plz_bl.dta", replace
...
use "$data_out/01_prices_germany", clear
...
* Hourly
use "$data_out/02_frenchdata_2020_hourly", clear
append using "$data_out/01_prices_germany_hourly.dta"
...
* Save
save "$data_out/final_hourly"


* Daily weighted
use "$data_out/02_frenchdata_2020_daily", clear
append using "$data_out/01_prices_germany_weighted.dta"
...
* Save
save "$data_out/final_weighted"
	

* Daily
use "$data_out/02_frenchdata_2020_daily", clear
append using "$data_out/01_prices_germany_daily.dta"
