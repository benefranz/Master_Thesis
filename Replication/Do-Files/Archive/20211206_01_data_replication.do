...
*** Apend Data
use "$data_out/Prices/2020-06-15-prices.dta", replace
forvalues mm = 16/30 {
	local m : di %02.0f `mm'	
...
*** Collapse data
collapse (mean) diesel e5 e10, by(id time)
...
*** Collapse data
collapse (mean) diesel e5 e10, by(id time)		// still keep postal street_type latitude longitude
...
*** Save
save "$data_out/05_communes_dep.dta", replace
...
replace sub_region_1 = subinstr(sub_region_1,"Ã¼","ü",.)
