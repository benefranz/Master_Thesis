...
forvalues ii=01/31{
	...
	save "$data_out/Prices/2020-12-`i'-prices.dta", replace
}
...
forvalues ii=01/31{
	...
	save "$data_out/Prices/2021-01-`i'-prices.dta", replace
}


* Apend Data
use "$data_out/Prices/2020-12-01-prices.dta", replace
forvalues mm = 02/31 {
	local m : di %02.0f `mm'	
	append using "$data_out/Prices/2020-12-`m'-prices.dta"
}
forvalues oo = 01/31{ 
	local o : di %02.0f `oo'	
	cap append using "$data_out/Prices/2021-01-`o'-prices.dta"
}
...
bys station_uuid (time): gen exp = cond(_n==_N, td(01-02-2021)*24-datehour, datehour[_n+1]-datehour)
expand exp
